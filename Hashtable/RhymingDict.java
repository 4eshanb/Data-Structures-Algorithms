...
public class RhymingDict {
	...
	public static void loadDictionary(DictionaryInterface rhymingDictionary) {
		...
		try {
			...
			String path = "cmudict-abridged.dict";
...
//		MyHashtable dictionary = new MyHashtable(20000);
		String word = getWord(line);
		String key = getRhymeGroup(line);
		// Object lookup = dictionary.get(key);
		MySortedLinkedList lookup = (MySortedLinkedList) rhymingDict.get(key);
		if (lookup == null) {
			MySortedLinkedList list = new MySortedLinkedList();
			list.add(word);
			rhymingDict.put(key, list);
	...
	public static void removeUnrhymables(DictionaryInterface rhymingDict) {
		String[] unRhymableWords = rhymingDict.getKeys();
		// MyLinkedList list = new MyLinkedList();
		for (int i = 0; i < unRhymableWords.length; i++) {
			String key = unRhymableWords[i];
			rhymingDict.get(key);
			if (unRhymableWords.length == 1) {
